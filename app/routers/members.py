...
def create_group(request: schemas.ServerItems, db: Session = Depends(get_db), current_user: schemas.UserItems = Depends(tokenmanager.get_current_user)):
    new_server = members.create_server_group(request, db, current_user)
    if new_server:
        raise HTTPException(status_code=status.HTTP_200_OK, detail={"success": True, "data": jsonable_encoder(
            request), "message": f"Group {request.server_group.lower()} created successfully"})
    raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail={
                        "success": False, "data": jsonable_encoder(request), "message": f"User {request.server_group.lower()} already exists. Use different name"})


@router.get('/accept-invite/{email}/{code}')
def accept_invite(code, email, db: Session = Depends(get_db)):
    check_invite = members.accept_invite(code, email, db)
    if check_invite:
